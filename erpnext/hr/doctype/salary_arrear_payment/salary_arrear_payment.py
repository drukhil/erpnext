...
class SalaryArrearPayment(Document):
	...
	def get_arrear_employees(self):
		...
		query = """
			SELECT
				...
				COALESCE(sd_pda.amount, 0) AS prev_pda,
				COALESCE(sd_shift_allowance.amount, 0) AS prev_shift_allowance,
				COALESCE(sd_underground_allowance.amount, 0) AS prev_underground_allowance
			FROM tabEmployee e
			...
			LEFT JOIN `tabSalary Detail` sd_pda ON sd_pda.parent = ss.name AND sd_pda.salary_component = 'PDA'
			LEFT JOIN `tabSalary Detail` sd_shift_allowance ON sd_shift_allowance.parent = ss.name AND sd_shift_allowance.salary_component = 'Shift Allowance'
			LEFT JOIN `tabSalary Detail` sd_underground_allowance ON sd_underground_allowance.parent = ss.name AND sd_underground_allowance.salary_component = 'Underground Allowance'
			WHERE NOT EXISTS (
				SELECT 1
		...
		for d in entries:
			...
			if d.prev_pda > 0:
				if sal_struct.pda_method == "Percent":
					d.pda = flt(d.basic_pay*(sal_struct.pda*0.01),0)
				elif sal_struct.pda_method == "Lumpsum":
					d.pda = flt(sal_struct.pda)
			if d.prev_shift_allowance > 0:
				if sal_struct.shift_method == "Percent":
					d.shift = flt(d.basic_pay*(sal_struct.shift*0.01),0)
				elif sal_struct.shift_method == "Lumpsum":
					d.shift = flt(sal_struct.shift)
			if d.prev_underground_allowance > 0:
				if sal_struct.underground_method == "Percent":
					d.underground = flt(d.basic_pay*(sal_struct.underground*0.01),0)
				elif sal_struct.underground_method == "Lumpsum":
					d.underground = flt(sal_struct.underground)
			...
			d.arrear_pda = d.pda - d.prev_pda
			d.arrear_shift_allowance = d.shift - d.prev_shift_allowance
			d.arrear_underground_allowance = d.underground - d.prev_underground_allowance
			d.salary_tax = get_salary_tax((d.basic_pay+d.corporate_allowance+d.contract_allowance+d.fixed_allowance + d.project_allowance + d.pda + d.shift + d.underground- d.pf))
			d.health_contribution = flt((d.basic_pay+d.corporate_allowance+d.contract_allowance+d.fixed_allowance+ d.project_allowance + d.pda + d.shift + d.underground) * (d.health_con_per * 0.01),0)
			d.arrear_pf = flt(d.pf-d.previous_pf)
			d.arrear_employer_pf = flt(d.employer_pf-d.previous_employer_pf)
			d.new_gross_pay = flt(d.arrear_basic_pay + d.arrear_corporate_allowance + d.arrear_contract_allowance + d.arrear_officiating_allowance + d.fixed_allowance + d.arrear_project_allowance + d.arrear_pda + d.arrear_shift_allowance + d.arrear_underground_allowance)
	...
	def make_accounting_entry(self):
			...
			if cost_center not in cc:
				cc.update({
        			cost_center: {
						...
						"pda": det.arrear_pda,
						"shift": det.arrear_shift_allowance,
						"underground": det.arrear_underground_allowance,
				...
				cc[cost_center]['pda'] += det.arrear_pda
				cc[cost_center]['shift'] += det.arrear_shift_allowance
				cc[cost_center]['underground'] += det.arrear_underground_allowance
		...
		for rec in cc:
			...
			#PDA
			if flt(cc[rec]['pda'],2) > 0:
				payables_je.append("accounts", {
						"account": frappe.db.get_value("Salary Component", "PDA", "gl_head"),
						"reference_type": self.doctype,
						"reference_name": self.name,
						"cost_center": rec,
						"debit_in_account_currency": flt(cc[rec]['pda'],2),
						"debit": flt(cc[rec]['pda'],2),
					})
			
			#Shift Allowance
			if flt(cc[rec]['shift'],2) > 0:
				payables_je.append("accounts", {
						"account": frappe.db.get_value("Salary Component", "Shift Allowance", "gl_head"),
						"reference_type": self.doctype,
						"reference_name": self.name,
						"cost_center": rec,
						"debit_in_account_currency": flt(cc[rec]['shift'],2),
						"debit": flt(cc[rec]['shift'],2),
					})
			
			#Underground Allowance
			if flt(cc[rec]['underground'],2) > 0:
				payables_je.append("accounts", {
						"account": frappe.db.get_value("Salary Component", "Underground Allowance", "gl_head"),
						"reference_type": self.doctype,
						"reference_name": self.name,
						"cost_center": rec,
						"debit_in_account_currency": flt(cc[rec]['underground'],2),
						"debit": flt(cc[rec]['underground'],2),
					})
			#Fixed Allowance
			payables_je.append("accounts", {
			...
			#Total Allowance
			total_allowance += flt(cc[rec]['corporate_allowance'],2)+flt(cc[rec]['contract_allowance'],2)+flt(cc[rec]['project_allowance'],2)+flt(cc[rec]['fixed_allowance'],2)+flt(cc[rec]['pda'],2)+flt(cc[rec]['shift'],2)+flt(cc[rec]['underground'],2)
		#Health Contribution
		payables_je.append("accounts", {
		...
		pf_je.title = "Arrear PF contribution of CDCL staff for the month of "+self.from_month
		pf_je.remark =  "Arrear PF contribution of CDCL staff for the month of "+self.from_month
