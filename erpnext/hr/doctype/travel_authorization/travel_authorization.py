	...
	def check_double_dates(self):
		if self.items:
			...
			if las:
				...
				leave_from_date, leave_to_date = frappe.db.get_value("Leave Application", las[0]['name'], ["from_date","to_date"])
				quarantine_flag = 0
				for d in self.items:
...
def get_permission_query_conditions(user):
	...
	if user == "Administrator":
		return
	if "HR Master" in user_roles or "Accounts Manager" in user_roles:
		return
	if "HR User" in user_roles or "HR Manager" in user_roles:
		...
		return """(
			`tabTravel Authorization`.branch = '{branch}'
