...
class SalaryStructure(Document):
	...
	def validate_amount(self):
		...
		if not self.oneoff_fixed_payment_gi and self.employment_type == "Contract":
			self.oneoff_fixed_payment_gi = frappe.db.get_value("Employee Grade", self.employee_grade, "fixed_payment")
	...
	def update_salary_structure(self, new_basic_pay=0):
		...
		basic_pay = comm_allowance = gis_amt = pf_amt = health_cont_amt = tax_amt = payscale_lower_limit = 0
		...
		payscale_lower_limit = frappe.db.get_value("Employee Grade", frappe.db.get_value("Employee",self.employee,"employee_subgroup"), "minimum")
